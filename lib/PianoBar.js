...
class PianoBar {
    ...
    _updateCurrentStation(station) {
        ...
        if(!station){ return; }
        if(!self.stations){ return; }
        if(!self.currentStation){ self.currentStation = { name: 'station0', title: 'Station Zero' }; }
        station = station.trim();

        if(station === self.currentStation.name || station === self.currentStation.title){ return; }
