...
function is_charging() {
    local bat_exist=$1
    local charge_ind=$2
    local has_discharge_current=$(echo " $3 > 0" | bc)
    local has_charge_current=$(echo " $4 > 0" | bc)

    if [ ${charge_ind} ] || ([ ${has_discharge_current} -eq 0 ] && [ ${has_charge_current} -gt 0 ]); then
        echo "0";
    else
        echo "1";
    fi
...
if [ ! -e /usr/local/battery_stats ]; then
    touch /usr/local/battery_stats
    chmod 755 /usr/local/battery_stats
fi
...
# echo $BAT_EXIST
if [ ${BAT_EXIST} -eq 0 ]; then
    echo "0|0|0|0|0" > /sys/battery_stats
    exit 0
fi
...
if [ `is_charging ${BAT_EXIST} ${CHARG_IND} ${BAT_IDISCHG} ${BAT_ICHG}` ]; then
    CHARGING=1
fi

# Has Battery, Is Charging, Voltage (mV), Discharge Current (mA), Percentage of Battery
echo "1|${CHARGING}|${BAT_VOLT}|${BAT_IDISCHG}|${PERC_CHG}" > /usr/local/battery_stats
